...
#include <mavros_msgs/WheelEncodersStamped.h>
 ...
 *
 */
class WheelEncodersPlugin : public plugin::PluginBase {
public:
	EIGEN_MAKE_ALIGNED_OPERATOR_NEW

	WheelEncodersPlugin() : PluginBase(),
		wo_nh("~wheel_encoders"),
		count(0),
		odom_mode(OM::NONE),
		raw_send(false),
		twist_send(false),
	...
	void initialize(UAS &uas_)
	{
		...
		if (raw_send) {
			...
			enc_pub = wo_nh.advertise<mavros_msgs::WheelEncodersStamped>("encoders", 10);
		}
	...
	{
		return {
			make_handler(&WheelEncodersPlugin::handle_encoders),
			// make_handler(&WheelEncodersPlugin::handle_wheel_distance)
		};
	}
...
private:
	 ...
	 */
	void handle_encoders(const mavlink::mavlink_message_t *msg, mavlink::common::msg::WHEEL_ENCODERS &wheel_enc)
	{
		// Get ROS timestamp of the message
		...
		// Publish distances
		if (raw_send) {
			auto wheel_enc_msg = boost::make_shared<mavros_msgs::WheelEncodersStamped>();

			wheel_enc_msg->header.stamp = timestamp;
			// wheel_enc_msg->positions.resize(2);
			// wheel_enc_msg->pps.resize(2);
			// wheel_enc_msg->ppr.resize(2);
			std::copy_n(wheel_enc.encoder_position.begin(), 2, wheel_enc_msg->positions.begin());
			std::copy_n(wheel_enc.speed.begin(), 2, wheel_enc_msg->pps.begin());
			std::copy_n(wheel_enc.pulses_per_rev.begin(), 2, wheel_enc_msg->ppr.begin());
...
PLUGINLIB_EXPORT_CLASS(mavros::extra_plugins::WheelEncodersPlugin, mavros::plugin::PluginBase)
